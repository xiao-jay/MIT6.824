...
func TestReElection2A(t *testing.T) {
	for i := 0; i < 10; i++ {
		...
		DPrintf("check leader 1")
		leader1 := cfg.checkOneLeader()

		// if the leader disconnects, a new one should be elected.
		cfg.disconnect(leader1)
		DPrintf("check leader 2")
		cfg.checkOneLeader()

		// if the old leader rejoins, that shouldn't
		// disturb the new leader.
		cfg.connect(leader1)
		DPrintf("check leader 3")
		...
		DPrintf("check leader 4")
		cfg.checkOneLeader()

		// re-join of last node shouldn't prevent leader from existing.
		cfg.connect(leader2)
		DPrintf("check leader 5")
		cfg.checkOneLeader()
...
func TestManyElections2A(t *testing.T) {
	...
	for ii := 1; ii < iters; ii++ {
		...
		cfg.disconnect(i2)
		cfg.disconnect(i3)
		var connectServers []int
		for i := 0; i < servers; i++ {
			if cfg.connected[i] {
				connectServers = append(connectServers, i)
			}
		}
		DPrintf("connect service %v", connectServers)
